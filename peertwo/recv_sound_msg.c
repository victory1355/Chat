...
void *recv_sound_routine(void *arg)
{
	...
	addr.sin_addr.s_addr = inet_addr("192.168.114.60"); // 绑定本机IP
	addr.sin_port   = htons(53000); // 端口号，范围: 1 - 65535，最好用50000以上的端口防冲突

	// ２，绑定地址（IP + PORT）
	int intb = bind(fd, (struct sockaddr *)&addr, len);
	if(intb == -1)
	{
		perror("recv_sound_routine bind failed");
		close(fd);
		return NULL;
	}
	// ３，将套接字设置为监听状态
	...
	if(retl == -1)
	{
		perror("recv_sound_routine listen failed");
		close(fd);
		return NULL;
	}
	// ４，坐等对方的连接请求... ...
	...
	if(newwavfd==-1)
	{
		perror("create file recv.wav failed\n");
		close(connfd);
		close(fd);
		return NULL;
	}
