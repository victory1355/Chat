		...
		if((x>0 && x<100) && (y>270 && y<480))
		{
			...
			if(inpt.type == EV_KEY && inpt.code == BTN_TOUCH)
			{
				if(inpt.value > 0)
				{
					...
					{
						printf("set up sound tcp connection\n");
						//1，第一次就建立TCP连接
...
void *recv_routine(void *arg)
{
	...
	bindaddr.sin_addr.s_addr = inet_addr("192.168.114.100");//李四自己的ip
	bindaddr.sin_port = htons(55500);
...
void *send_routine(void *arg)
{
	...
	socklen_t len = sizeof(bindaddr);
	bindaddr.sin_family = AF_INET;
	bindaddr.sin_addr.s_addr = inet_addr("192.168.114.100");
